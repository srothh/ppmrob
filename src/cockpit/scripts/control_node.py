...
class DroneControl:
    def __init__(self):
        self.prev = Point(0.0, 0.0, 0.0)
        self.course = 0.0
        self.launched = False
        rospy.init_node("control_node", anonymous=True)

        self._feedback = control.msg.PlanningMoveFeedback()
        self._result = control.msg.PlanningMoveResult()

        # Action Server
        self._as = actionlib.SimpleActionServer(
            config.Control.MOVE_ACTION_NAMESPACE,
            control.msg.PlanningMoveAction,
            execute_cb=self.execute_cb,
            auto_start=False
        )

        self.drone_launch_client = actionlib.SimpleActionClient("launch", drone.msg.LaunchAction)
        rospy.loginfo("waiting for drone action client")
